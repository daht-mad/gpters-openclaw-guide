...
> 이미 Docker가 설치되어 있다면 바로 시작!

```bash
# 1. OpenClaw 다운로드
cd ~/Documents && git clone https://github.com/openclaw/openclaw.git && cd openclaw
...
9. [문제 해결](#9-문제-해결)
10. [유용한 명령어](#10-유용한-명령어)
11. [네이티브로 전환하기 (선택)](#11-네이티브로-전환하기-선택)
...
> 실제 브라우저 창을 보면서 작업하려면 [네이티브 설치](#11-네이티브로-전환하기-선택)를 권장합니다.
...
**더 많은 기능이 필요하면?** → [네이티브로 전환하기](#11-네이티브로-전환하기-선택)
...
**칩 종류 확인:**
1. 🍎 애플 로고 → "이 Mac에 관하여"
2. "칩"이면 Apple Silicon, "프로세서"면 Intel
...
> 💡 Git이 없다면: `xcode-select --install` 실행
...
**Step 1: Claude Code CLI 설치** (로컬 터미널에서)
...
**Step 2: setup-token 생성** (로컬 터미널에서)
...
**Step 3: OpenClaw에 토큰 등록** (Docker에서)
...
**해결:** Docker Desktop 실행 → 🐳 아이콘이 멈출 때까지 대기
...
- [ ] Docker Desktop 설치됨 (메뉴바에 🐳 아이콘)
- [ ] `docker --version` 작동
- [ ] OpenClaw 폴더: `~/Documents/openclaw`

...
> 💡 **팁**: 확장 프로그램을 기본으로 쓰고 싶다면 `browser.defaultProfile`을 `chrome`으로 설정하고, 사용 전에 Chrome에서 탭을 attach하세요.

---

### 🎯 프로필 선택 가이드: 언제 어떤 프로필을 쓸까?

OpenClaw는 **자동으로 프로필을 선택하지 않습니다**. 명시하지 않으면 기본 프로필(`defaultProfile`)을 사용합니다.

#### 프로필별 사용 시점

| 상황 | 추천 프로필 | 이유 |
|------|------------|------|
| **로그인 필요** (구글캘린더, Gmail, Notion, 회사 서비스) | `chrome` | 기존 로그인 세션 활용 |
| **2FA/SSO 인증된 사이트** (은행, AWS, GitHub 조직) | `chrome` | 이미 인증된 상태 |
| **개인화된 데이터 조회** (내 유튜브, 내 트위터) | `chrome` | 쿠키/세션 필요 |
| **공개 정보 수집** (뉴스, 가격 비교, 검색) | `openclaw` | 격리된 환경, 안전 |
| **테스트/스크래핑** | `openclaw` | 깨끗한 상태, 반복 가능 |
| **스크린샷/캡처** | `openclaw` | 로그인 배너 없는 깔끔한 화면 |
...
# Chrome 확장 프로그램 (로그인된 브라우저)
"크롬 프로필로 구글캘린더에서 오늘 일정 알려줘"
"profile chrome으로 Gmail 확인해"

# OpenClaw 관리형 (격리된 브라우저)
"openclaw 프로필로 스크린샷 찍어"
"새 브라우저로 이 URL 열어줘"
```
...
```bash
# 안전을 위해 기본값을 openclaw(격리)로 설정
openclaw config set browser.defaultProfile openclaw

# 로그인 필요할 때만 "크롬 프로필로" 명시
```

...
브라우저 자동화에서 **프로필 선택**은 중요합니다:

| 상황 | 추천 프로필 | 이유 |
|------|------------|------|
| **로그인 필요** (구글캘린더, Gmail, 네이버 등) | `chrome` (확장 프로그램 방식) | 기존 로그인 세션 그대로 활용 |
| **공개 정보 수집, 테스트** | `openclaw` (기본) | 격리된 환경, 개인정보 노출 없음 |
| **민감한 작업** (결제, 계정 설정) | `chrome` + 직접 확인 | AI가 조작하되 최종 확인은 사람이 |

#### 요청 시 프로필 지정하는 법

```
# 기본 프로필(openclaw) 사용 - 별도 명시 불필요
"네이버에서 날씨 검색해줘"

# 로그인 필요시 - 명시적으로 요청
"크롬 프로필로 Gmail 열어서 안 읽은 메일 확인해줘"
"내 크롬으로 구글 캘린더 오늘 일정 알려줘"
```

#### 추천 설정

```powershell
# 기본 프로필을 openclaw로 설정 (안전)
openclaw config set browser.defaultProfile openclaw

# 로그인이 필요한 작업만 "크롬 프로필로"라고 명시
```

> 💡 **팁**: 확장 프로그램 방식(방법 1)을 쓰면 Chrome의 모든 로그인 상태를 그대로 활용할 수 있어서, 로그인이 필요한 자동화에 훨씬 편리합니다!
